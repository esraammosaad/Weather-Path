...
class HomeViewModelImpl(private val repository: Repository) : ViewModel() {
    ...
    private var _firstDayWeatherForecast: MutableLiveData<List<WeatherItem>> =
        MutableLiveData()
    var firstDayWeatherForecast: LiveData<List<WeatherItem>> =
        _firstDayWeatherForecast
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Log.i(
                        "TAG",
                        "getCurrentWeather: ======sss ${response.body()?.weather?.get(0)?.icon}"
                    )
    ...
    fun getFiveDaysWeatherForecast(latitude: Double, longitude: Double) {
        ...
        viewModelScope.launch {
            try {
                ...
                _currentDayList.value=response.filter { formatDateTime(it.dt_txt) == getCurrentDate(0) }.toList()
                _nextDayList.value=response.filter { formatDateTime(it.dt_txt) == getCurrentDate(1) }.toList()
                _thirdDayList.value=response.filter { formatDateTime(it.dt_txt) == getCurrentDate(2) }.toList()
                _fourthDayList.value=response.filter { formatDateTime(it.dt_txt) == getCurrentDate(3) }.toList()
                _fifthDayList.value=response.filter { formatDateTime(it.dt_txt) == getCurrentDate(4) }.toList()
                _sixthDayList.value=response.filter { formatDateTime(it.dt_txt) == getCurrentDate(5) }.toList()
